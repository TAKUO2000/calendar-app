...
const CalenderDialog = ({
  ...
  return (
    <>
      <Dialog open={openDate} onClose={setOpenDate} className="relative z-10">
        <DialogBackdrop
          transition
                      ...
                      {dateSchedule.length > 0 ? (
                        <>
                          <div className="my-2 p-2 border-gray-400 border-b-2 border-soli">
                            ...
                            {dateSchedule.map(
                              (item) =>
                                item.start.substr(0, 10) === selectedDate && (
                                  <DisplaySchedule
                                    key={item.id}
                                    item={item}
                          ...
                          <h3 className="text-gray-500">他の予定</h3>
                          {dateSchedule.map(
                            (item) =>
                              item.start.substr(0, 10) !== selectedDate && (
                                <DisplaySchedule
                        ...
                        <h2 className="text-gray-500">予定なし</h2>
                      )}
                    </div>
                    {openInput && (
                      <>
