...
const DisplaySchedule = ({ item, setDateSchedule }) => {
  ...
  const submitEditEvent = async () => {
    ...
    try {
      const response = await scheduleApi.patch(editEvent);
      setDateSchedule((prevstate) =>
        prevstate.map((event) => (event.id === response.id ? response : event))
      );
      alert(`${response.title} 編集完了`);
      ...
      {editting ? (
        <>
          <div className="my-2 p-2 border-red-400 rounded-lg border-2 border-solid">
            <div className="mr-1 h-6 w-6 shrink-0 items-center justify-center rounded-full sm:mx-0 sm:h-10 sm:w-10">
              <AdjustmentsHorizontalIcon onDoubleClick={toggelEditting} />
            </div>
            <form>
              <input
        ...
        <>
          <div className="my-2 p-2 border-gray-400 rounded-lg border-2 border-solid">
            ...
            <p className="text-sm text-gray-500">詳細：{item.description}</p>
